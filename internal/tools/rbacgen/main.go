...
func run(filename, outfile, enumOutfilePath, operationEnumOutfile, operationCostOutfile string) error {
	...
	if err := enumOutfile.Close(); err != nil {
		return err
	}
	if err := writePermissionList(filepath.Join(filepath.Dir(enumOutfilePath), "permission_list_gen.go"), enum.Values); err != nil {
		return err
	}
...
func writePermissionList(outfile string, values []generate.Value) error {
	f := jen.NewFile("rbac")
	f.HeaderComment("Code generated by rbacgen. DO NOT EDIT.")

	allPermissions := make([]jen.Code, 0, len(values))
	for _, value := range values {
		allPermissions = append(allPermissions, jen.Id(value.Symbol))
	}

	f.Var().Id("AllPermissions").Op("=").Id("PermissionList").Values(allPermissions...)
