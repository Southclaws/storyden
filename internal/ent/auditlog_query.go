...
// QueryEnactedBy chains the current query on the "enacted_by" edge.
func (_q *AuditLogQuery) QueryEnactedBy() *AccountQuery {
...
func (_q *AuditLogQuery) FirstID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (_q *AuditLogQuery) FirstIDX(ctx context.Context) xid.ID {
...
func (_q *AuditLogQuery) OnlyID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (_q *AuditLogQuery) OnlyIDX(ctx context.Context) xid.ID {
...
func (_q *AuditLogQuery) IDs(ctx context.Context) (ids []xid.ID, err error) {
...
func (_q *AuditLogQuery) IDsX(ctx context.Context) []xid.ID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *AuditLogQuery) Clone() *AuditLogQuery {
	...
	return &AuditLogQuery{
		...
		withEnactedBy: _q.withEnactedBy.Clone(),
...
// WithEnactedBy tells the query-builder to eager-load the nodes that are connected to
// the "enacted_by" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AuditLogQuery) WithEnactedBy(opts ...func(*AccountQuery)) *AuditLogQuery {
	query := (&AccountClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withEnactedBy = query
	return _q
}
...
func (_q *AuditLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuditLog, error) {
	...
	if query := _q.withEnactedBy; query != nil {
		if err := _q.loadEnactedBy(ctx, query, nodes, nil,
			func(n *AuditLog, e *Account) { n.Edges.EnactedBy = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AuditLogQuery) loadEnactedBy(ctx context.Context, query *AccountQuery, nodes []*AuditLog, init func(*AuditLog), assign func(*AuditLog, *Account)) error {
	ids := make([]xid.ID, 0, len(nodes))
	nodeids := make(map[xid.ID][]*AuditLog)
	for i := range nodes {
		if nodes[i].EnactedByID == nil {
			continue
		}
		fk := *nodes[i].EnactedByID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "enacted_by_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *AuditLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withEnactedBy != nil {
			_spec.Node.AddColumnOnce(auditlog.FieldEnactedByID)
		}
	}
