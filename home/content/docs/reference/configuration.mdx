...
<table>
<tr><td>type</td><td>duration (e.g. 1h, 1m, 1s)</td></tr>
<tr><td>default</td><td>`720h`</td></tr>
</table>
...
### `ROBOT_WORKSPACE_DATA_PATH`

<table>
<tr><td>type</td><td>`string`</td></tr>
<tr><td>default</td><td>`./data/robot-workspaces`</td></tr>
</table>
...
Configuration for optional AI features. These can be useful for organising large amounts of library pages and threads, but it can also provide other features such as recommendations and chat-based conversational searching.
...
The Semdex is a semantic index that provides vector-based storage of content. This is used for things like recommendations, search, etc.
 ...
 The Semdex works with the OpenAI Robot model provider to create embeddings of content. Thus, enabling the Semdex requires Robots settings to include an enabled OpenAI provider with an API key.
