...
There's one secret that you need to set for your deployment to be secure: `SESSION_KEY`
...
<Callout>
  An example value is intentionally omitted here. You should *always* generate
  cryptographic source secrets yourself.
</Callout>

Then apply this value to the `SESSION_KEY` secret:

```sh
fly secrets set SESSION_KEY=yoursupersecretvalue
```
