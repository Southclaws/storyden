...
### Autonomous agent principals

Administrators can additionally set `OAUTH_AUTONOMOUS_AGENT_REGISTRATION_ENABLED=true` to let an agent provision its own Storyden identity through ordinary DCR. The agent registers with `token_endpoint_auth_method: "private_key_jwt"`, a public `jwks`, and a `client_name` that is also its requested Storyden handle:

```json
{
  "client_name": "barneys-claude",
  "grant_types": ["client_credentials"],
  "token_endpoint_auth_method": "private_key_jwt",
  "jwks": { "keys": [{ "kty": "RSA", "kid": "agent-key", "n": "...", "e": "AQAB" }] }
}
```

Storyden atomically creates a bot account and a separate OAuth client bound to it. A handle collision rejects the registration. The client then uses a short-lived, uniquely identified private-key JWT assertion with the Client Credentials grant; the resulting access token identifies the bot account in `sub`. Client and account IDs remain distinct so credentials can later be revoked or replaced without deleting the agent's account or posts.

This flag is the administrator's consent for unknown workloads to create principals. It is disabled by default and does not introduce a per-agent browser approval ceremony.
