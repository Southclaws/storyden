	...
	integration.Test(t, nil, e2e.Setup(), fx.Invoke(func(
		...
		lc.Append(fx.StartHook(func() {
			t.Run("lists_sent_signup_email", func(t *testing.T) {
				...
				require.True(t, strings.HasPrefix(email.Subject, welcomeSubjectPrefix))
				...
				require.Equal(t, email.Subject, last.Subject)
			})
			...
			t.Run("lists_failed_password_reset_email", func(t *testing.T) {
				...
				require.True(t, strings.HasPrefix(welcomeEmail.Subject, welcomeSubjectPrefix))
				require.Equal(t, 1, inbox.Count())
				...
				require.True(t, strings.HasPrefix(email.Subject, passwordResetSubjectPrefix))
			...
			t.Run("filters_by_status_query_and_date_range", func(t *testing.T) {
				inbox.Reset()

				adminCtx, _ := e2e.WithAccount(root, aw, seed.Account_001_Odin)
				adminSession := sh.WithSession(adminCtx)

				sentAddress := uniqueEmailAddress()
				failedAddress := uniqueEmailAddress()

				signupEmailOnly(t, root, cl, sentAddress)
				sentRow := waitForEmailQueueRow(t, adminCtx, db, func(row *ent.EmailQueue) bool {
					return row.RecipientAddress == sentAddress &&
						row.Status == ent_emailqueue.StatusSent
				})

				inbox.SetSendError(errors.New("simulated filtering failure"))
				t.Cleanup(inbox.ClearSendError)

				signupEmailOnly(t, root, cl, failedAddress)
				failedRow := waitForEmailQueueRow(t, adminCtx, db, func(row *ent.EmailQueue) bool {
					return row.RecipientAddress == failedAddress &&
						row.Status == ent_emailqueue.StatusFailed
				})

				statuses := []openapi.EmailQueueStatus{openapi.EmailQueueStatusFailed}
				failedOnly := waitForEmailQueueListWithParams(t, adminCtx, cl, adminSession, &openapi.EmailQueueListParams{
					Statuses: &statuses,
				}, func(items []openapi.EmailQueueItem) bool {
					return len(items) >= 1 &&
						indexOfEmail(items, func(item openapi.EmailQueueItem) bool {
							return string(item.RecipientAddress) == failedAddress &&
								item.Status == openapi.EmailQueueStatusFailed
						}) >= 0
				})
				require.NotEmpty(t, failedOnly)
				require.Equal(t, failedAddress, string(failedOnly[0].RecipientAddress))
				require.Equal(t, openapi.EmailQueueStatusFailed, failedOnly[0].Status)

				search := failedAddress
				searched := waitForEmailQueueListWithParams(t, adminCtx, cl, adminSession, &openapi.EmailQueueListParams{
					Q: &search,
				}, func(items []openapi.EmailQueueItem) bool {
					return len(items) == 1 &&
						string(items[0].RecipientAddress) == failedAddress
				})
				require.Len(t, searched, 1)
				require.Equal(t, failedAddress, string(searched[0].RecipientAddress))

				start := sentRow.CreatedAt
				if failedRow.CreatedAt.Before(start) {
					start = failedRow.CreatedAt
				}
				end := sentRow.CreatedAt
				if failedRow.CreatedAt.After(end) {
					end = failedRow.CreatedAt
				}

				rangeValue := start.Add(-time.Minute).Format(time.RFC3339) + "/" + end.Add(time.Minute).Format(time.RFC3339)
				ranged := waitForEmailQueueListWithParams(t, adminCtx, cl, adminSession, &openapi.EmailQueueListParams{
					Range: &rangeValue,
				}, func(items []openapi.EmailQueueItem) bool {
					return indexOfEmail(items, func(item openapi.EmailQueueItem) bool {
						return string(item.RecipientAddress) == sentAddress
					}) >= 0 &&
						indexOfEmail(items, func(item openapi.EmailQueueItem) bool {
							return string(item.RecipientAddress) == failedAddress
						}) >= 0
				...
				startOnlyRange := start.Add(-time.Minute).Format(time.RFC3339) + "/"
				openEnded := waitForEmailQueueListWithParams(t, adminCtx, cl, adminSession, &openapi.EmailQueueListParams{
					Range: &startOnlyRange,
				}, func(items []openapi.EmailQueueItem) bool {
					return indexOfEmail(items, func(item openapi.EmailQueueItem) bool {
						return string(item.RecipientAddress) == sentAddress
					}) >= 0 &&
...
func waitForEmailQueueItem(
	...
	waitForEmailQueueListWithParams(t, ctx, cl, session, &openapi.EmailQueueListParams{}, func(items []openapi.EmailQueueItem) bool {
...
func waitForEmailQueueList(
	...
	return waitForEmailQueueListWithParams(t, ctx, cl, session, &openapi.EmailQueueListParams{}, match)
}

func waitForEmailQueueListWithParams(
	t *testing.T,
	ctx context.Context,
	cl *openapi.ClientWithResponses,
	session openapi.RequestEditorFn,
	params *openapi.EmailQueueListParams,
	match func([]openapi.EmailQueueItem) bool,
) []openapi.EmailQueueItem {
	t.Helper()
	...
	require.Eventually(t, func() bool {
		list, err := cl.EmailQueueListWithResponse(ctx, params, session)
