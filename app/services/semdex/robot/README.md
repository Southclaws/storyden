...
This directory contains the tool registry system for Storyden's AI agents (Robots). The architecture is schema-driven, with all tool definitions centralized in `/mcp/schema.yaml` and code generated from that single source of truth.

## Architecture Overview

### Schema-Driven Design

All tool definitions live in `/mcp/schema.yaml` as JSON Schema definitions. This provides:
...
### 1. Schema Definition (`/mcp/schema.yaml`)
...
### 2. Code Generation (`/mcp/embed.go`)
...
1. **Define schema in `/mcp/schema.yaml`**:
