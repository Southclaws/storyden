...
func (st *searchTools) ExecuteSearch(ctx tool.Context, args mcp.ToolSearchInput) ToolResult[mcp.ToolSearchOutput] {
	maxResults := 10
	...
	if len(args.Kinds) > 0 {
		kinds := make([]datagraph.Kind, 0, len(args.Kinds))
	...
	if err != nil {
		return NewError[mcp.ToolSearchOutput](err)
	}
	...
	if len(validationErrors) > 0 {
		return ToolResult[mcp.ToolSearchOutput]{
			Result: output,
			Error:  fmt.Sprintf("partial success with validation errors: %s", strings.Join(validationErrors, "; ")),
		}
	}
