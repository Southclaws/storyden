...
func (tt *threadTools) ExecuteThreadCreate(ctx tool.Context, args mcp.ToolThreadCreateInput) ToolResult[mcp.ToolThreadCreateOutput] {
	accountID, err := session.GetAccountID(ctx)
	if err != nil {
		return NewError[mcp.ToolThreadCreateOutput](fault.Wrap(err, fctx.With(ctx)))
	}
	...
	if err != nil {
		return NewError[mcp.ToolThreadCreateOutput](fault.Wrap(err, fctx.With(ctx), ftag.With(ftag.InvalidArgument)))
	}
	...
	if !found {
		return NewError[mcp.ToolThreadCreateOutput](fault.Wrap(fault.New("category not found"), fctx.With(ctx), ftag.With(ftag.InvalidArgument)))
	}
...
func (tt *threadTools) ExecuteThreadList(ctx tool.Context, args mcp.ToolThreadListInput) ToolResult[mcp.ToolThreadListOutput] {
	page := 1
	if args.Page != nil {
		page = *args.Page
	}
	...
	if err != nil {
		return NewError[mcp.ToolThreadListOutput](fault.Wrap(err, fctx.With(ctx)))
	}
...
func (tt *threadTools) ExecuteThreadGet(ctx tool.Context, args mcp.ToolThreadGetInput) ToolResult[mcp.ToolThreadGetOutput] {
	postID, err := tt.thread_mark_svc.Lookup(ctx, args.Slug)
	if err != nil {
		return NewError[mcp.ToolThreadGetOutput](fault.Wrap(err, fctx.With(ctx)))
	}
...
func (tt *threadTools) ExecuteThreadUpdate(ctx tool.Context, args mcp.ToolThreadUpdateInput) ToolResult[mcp.ToolThreadUpdateOutput] {
	postID, err := tt.thread_mark_svc.Lookup(ctx, args.Slug)
	if err != nil {
		return NewError[mcp.ToolThreadUpdateOutput](fault.Wrap(err, fctx.With(ctx)))
	}
	...
	if args.Body != nil && *args.Body != "" {
		richContent, err := datagraph.NewRichText(*args.Body)
		if err != nil {
			return NewError[mcp.ToolThreadUpdateOutput](fault.Wrap(err, fctx.With(ctx), ftag.With(ftag.InvalidArgument)))
		}
...
func (tt *threadTools) ExecuteThreadReply(ctx tool.Context, args mcp.ToolThreadReplyInput) ToolResult[mcp.ToolThreadReplyOutput] {
	accountID, err := session.GetAccountID(ctx)
	if err != nil {
		return NewError[mcp.ToolThreadReplyOutput](fault.Wrap(err, fctx.With(ctx)))
	}
	...
	richContent, err := datagraph.NewRichText(args.Body)
	if err != nil {
		return NewError[mcp.ToolThreadReplyOutput](fault.Wrap(err, fctx.With(ctx), ftag.With(ftag.InvalidArgument)))
	}
...
func (tt *threadTools) ExecuteCategoryList(ctx tool.Context, args map[string]any) ToolResult[mcp.ToolCategoryListOutput] {
	categories, err := tt.category_repo.GetCategories(ctx, false)
	if err != nil {
		return NewError[mcp.ToolCategoryListOutput](fault.Wrap(err, fctx.With(ctx)))
	}
