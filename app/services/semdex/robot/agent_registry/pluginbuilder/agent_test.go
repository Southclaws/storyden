...
func TestInstructionIncludesUnboundState(t *testing.T) {
	...
	assert.Contains(t, instruction, "Keep manifest events_consumed aligned with registered SDK event handlers")
	...
	assert.Contains(t, instruction, "Use context timeouts for outbound Storyden API calls")
	assert.Contains(t, instruction, "A 403 usually means manifest access.permissions is missing")
	...
	assert.Contains(t, instruction, "Treat integration secrets and external target identifiers as configuration")
	...
	assert.Contains(t, instruction, "Missing required configuration must not crash or exit")
	assert.Contains(t, instruction, "Validation failures are development work")
