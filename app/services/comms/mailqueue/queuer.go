...
func (q *Queuer) RetryNow(ctx context.Context, id email_queue.ID) (*email_queue.Email, error) {
	...
	// Keep the row out of the slow poller briefly while we trigger the
	// immediate command-path retry. The command handler can still claim the
	// pending row right away because claim does not depend on available_at.
	rec, err := q.queue.RetryNow(ctx, id, time.Now().Add(retryDelay))
	if err != nil {
