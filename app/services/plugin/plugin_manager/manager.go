...
func New(
	...
	provider, err := plugin_runner.ParseRuntimeProvider(cfg.PluginRuntimeProvider)
	pluginsEnabled := err != nil || provider != plugin_runner.RuntimeProviderNone
...
func (m *Manager) AddFromFile(ctx context.Context, r io.Reader) (*plugin.Available, error) {
	...
	b, err := readAllBounded(r, plugin.MaxArchiveSizeBytes, "plugin archive")
	if err != nil {
		return nil, fault.Wrap(err, fctx.With(ctx))
	}
...
func (m *Manager) AddFromURL(ctx context.Context, u url.URL) (*plugin.Available, error) {
	...
	b, err := fetchPluginArchive(ctx, u)
	if err != nil {
		return nil, fault.Wrap(err, fctx.With(ctx))
	}
...
func (m *Manager) onStart(ctx context.Context) error {
	if !m.pluginsEnabled {
		m.logger.Debug("plugins are disabled by configuration, skipping plugin startup")
		return nil
	}
...
func (m *Manager) requirePluginsEnabled(ctx context.Context) error {
	if m.pluginsEnabled {
		return nil
	}
