...
func (s *sdxSession) SetActiveState(ctx context.Context, state plugin.ActiveState) error {
	if s.supervisedRuntime == nil {
		...
		case plugin.ActiveStateActive:
			...
			if isConnected {
				s.reportState(plugin.ReportedStateActive, "")
			} else {
				s.reportState(plugin.ReportedStateConnecting, "")
			}
		...
		case plugin.ActiveStateInactive:
			...
			s.reportState(plugin.ReportedStateInactive, "")
...
func (s *sdxSession) reportState(state plugin.ReportedState, message string) {
	s.stateMu.Lock()
...
func (s *sdxSession) watchRuntimeEvents() {
	for event := range s.supervisedRuntime.Events() {
		s.reportState(event.State, event.Message)
	}
}
