...
func (r *runtime) startProcess(ctx context.Context) (*sprites.Cmd, *plugin_logger.RotatingWriter, error) {
	...
	logWriter, err := r.pluginLogger.NewWriter(r.id)
	if err != nil {
...
func (r *runtime) ensureSpriteSynced(ctx context.Context) (*sprites.Sprite, string, error) {
	...
	hash := archiveHash(r.bin)
	workdir := spriteWorkdir
	markerPath := path.Join(workdir, deploymentMarker)

	if !created && r.deployedArchiveHash == hash {
		r.logger.Debug("sprite plugin deployment already present",
			slog.String("workdir", workdir),
	...
	r.logger.Debug("clearing sprite workdir", slog.String("workdir", workdir))
	if err := fsys.RemoveAllContext(ctx, workdir); err != nil && !errors.Is(err, fs.ErrNotExist) {
	...
	stats, err := extractSDXArchiveToSprite(ctx, fsys, r.bin, workdir)
	if err != nil {
		return nil, "", fault.Wrap(err, fmsg.With("failed to write plugin archive to sprite filesystem"))
	}

	if err := fsys.WriteFileContext(ctx, markerPath, []byte(hash+"\n"), 0o644); err != nil {
		return nil, "", fault.Wrap(err, fmsg.With("failed to write sprite deployment marker"))
	}
...
func archiveHash(bin []byte) string {
...
func extractSDXArchiveToSprite(ctx context.Context, fsys sprites.FS, bin []byte, workdir string) (archiveExtractStats, error) {
	...
	for _, file := range zr.File {
		if !file.FileInfo().IsDir() {
			fileCount++
			if fileCount > plugin.MaxArchiveExtractFileCount {
				return stats, fault.Newf("archive contains too many files (max: %d)", plugin.MaxArchiveExtractFileCount)
			}
			if file.UncompressedSize64 > plugin.MaxArchiveExtractFileBytes {
				return stats, fault.Newf("archive file %q exceeds max size (%d bytes)", file.Name, plugin.MaxArchiveExtractFileBytes)
			}
			if totalUncompressed+file.UncompressedSize64 > plugin.MaxArchiveExtractTotalBytes {
				return stats, fault.Newf("archive exceeds max total extracted size (%d bytes)", plugin.MaxArchiveExtractTotalBytes)
			}
		...
		if uint64(len(data)) != file.UncompressedSize64 {
			return stats, fault.Newf("unexpected extracted size for file %s (read %d bytes, expected %d)", file.Name, len(data), file.UncompressedSize64)
		}
		...
		stats.Files++
		stats.Bytes += int64(file.UncompressedSize64)
	}
