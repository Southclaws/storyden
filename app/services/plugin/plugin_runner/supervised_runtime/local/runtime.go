...
func (r *localRuntime) startProcess(ctx context.Context) (*exec.Cmd, *plugin_logger.RotatingWriter, error) {
	workdir, err := r.ensureExtractedSDXArchive()
	if err != nil {
...
func (r *localRuntime) ensureExtractedSDXArchive() (string, error) {
	pluginDir := filepath.Join(r.dataPath, r.id.String())
	if err := os.MkdirAll(pluginDir, 0o755); err != nil {
		return "", fault.Wrap(err, fmsg.With("failed to create plugin directory"))
	}

	hash := archiveHash(r.bin)
	if r.deployedArchiveHash == hash {
	...
	if err := extractSDXArchiveToDir(r.bin, pluginDir); err != nil {
		return "", err
	}

	if err := os.WriteFile(markerPath, []byte(hash+"\n"), 0o644); err != nil {
		return "", fault.Wrap(err, fmsg.With("failed to write deployment marker"))
	}
	...
	for _, file := range zr.File {
		if !file.FileInfo().IsDir() {
			...
			if fileCount > plugin.MaxArchiveExtractFileCount {
				return fault.Newf("archive contains too many files (max: %d)", plugin.MaxArchiveExtractFileCount)
			}
			if file.UncompressedSize64 > plugin.MaxArchiveExtractFileBytes {
				return fault.Newf("archive file %q exceeds max size (%d bytes)", file.Name, plugin.MaxArchiveExtractFileBytes)
			}
			if totalUncompressed+file.UncompressedSize64 > plugin.MaxArchiveExtractTotalBytes {
				return fault.Newf("archive exceeds max total extracted size (%d bytes)", plugin.MaxArchiveExtractTotalBytes)
			}
		...
		n, err := io.Copy(out, io.LimitReader(rc, int64(file.UncompressedSize64)+1))
		if err != nil {
			out.Close()
			rc.Close()
		...
		if uint64(n) != file.UncompressedSize64 {
			out.Close()
			rc.Close()
			return fault.Newf("unexpected extracted size for file %s (wrote %d bytes, expected %d)", file.Name, n, file.UncompressedSize64)
		}
