...
func (b Binary) Validate(ctx context.Context) (*Validated, error) {
	if len(b) > int(MaxArchiveSizeBytes) {
		return nil, fault.Newf("archive exceeds maximum size of %d bytes", MaxArchiveSizeBytes)
	}
	...
	for _, file := range zr.File {
		...
		if file.UncompressedSize64 > uint64(MaxManifestSizeBytes) {
			return nil, fault.Newf("manifest exceeds maximum size of %d bytes", MaxManifestSizeBytes)
		}
		...
		limited := io.LimitReader(rc, MaxManifestSizeBytes+1)
