...
func newChatHandler(logger *slog.Logger, chatAgent *storydenagent.Agent) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		stream := chatAgent.Run(ctx, robotID, accountID.String(), sessionID, initMessage)
		...
		for event, streamErr := range stream {
			...
			if event != nil && event.LLMResponse.Content != nil {
				...
				if hasText && !textStarted {
					_ = emitter.Send(map[string]any{"type": "text-start", "id": textID})
					textStarted = true
				}
