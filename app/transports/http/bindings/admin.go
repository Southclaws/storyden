...
func (a *Admin) EmailQueueList(ctx context.Context, request openapi.EmailQueueListRequestObject) (openapi.EmailQueueListResponseObject, error) {
	...
	var statuses opt.Optional[[]email_queue.Status]
	if request.Params.Statuses != nil && len(*request.Params.Statuses) > 0 {
		parsedStatuses, err := dt.MapErr(*request.Params.Statuses, func(status openapi.EmailQueueStatus) (email_queue.Status, error) {
			return email_queue.NewStatus(string(status))
		})
		if err != nil {
			return nil, fault.Wrap(err, fctx.With(ctx), ftag.With(ftag.InvalidArgument))
		}
		statuses = opt.New(parsedStatuses)
	}

	var filterTimeRange opt.Optional[email_queue_querier.TimeRange]
	if request.Params.Range != nil {
		tr, err := timerange.Parse(*request.Params.Range)
		if err != nil {
			return nil, fault.Wrap(err, fctx.With(ctx), ftag.With(ftag.InvalidArgument))
		...
		if tr.Start.Ok() || tr.End.Ok() {
			filterTimeRange = opt.New(email_queue_querier.TimeRange{
				Start: tr.Start,
				End:   tr.End,
			})
		}
...
func (a *Admin) AccountList(ctx context.Context, request openapi.AccountListRequestObject) (openapi.AccountListResponseObject, error) {
	...
	if request.Params.Kind != nil {
		kind, err := account.NewAccountKind(string(*request.Params.Kind))
		if err != nil {
			return nil, fault.Wrap(err, fctx.With(ctx), ftag.With(ftag.InvalidArgument))
		}
		filters = append(filters, account_search.WithKind(kind))
	}
