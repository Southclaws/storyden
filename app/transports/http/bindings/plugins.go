...
func serialisePlugin(in *plugin.Record) openapi.Plugin {
	return openapi.Plugin{
		AddedAt:  in.Created,
		Status:   serialisePluginStatus(in.Status),
...
func serialisePluginStatus(in plugin.Status) openapi.PluginStatus {
	as := openapi.PluginStatus{}

	switch in.ActiveState {
	...
	default:
		as.FromPluginStatusError(openapi.PluginStatusError{
			...
			Details: map[string]any{
				"active_state": in.ActiveState,
				"details":      in.Details,
				"message":      in.StatusMessage,
				"changed_at":   in.ChangedAt,
			},
		})
	}
