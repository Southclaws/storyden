...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if author == "" {
				author = pluginapi.DefaultAuthor()
			}

			manifest := rpc.Manifest{
				ID:          id,
				Name:        pluginapi.Titleize(name),
				Author:      pluginapi.Slugify(author),
			...
			return pluginapi.WriteNewManifest(cmd.OutOrStdout(), dir, manifest, force)
		},
	}
