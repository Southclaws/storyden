...
import { InstanceCapability } from "@/api/openapi-schema/instanceCapability";
...
export function CommandPaletteProvider({ children }: PropsWithChildren) {
  ...
  const robotsEnabled = useCapability(InstanceCapability.robots);
  const canUseRobots =
    robotsEnabled && hasPermission(session, "USE_ROBOTS", "MANAGE_ROBOTS");
