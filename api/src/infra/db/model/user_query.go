...
func (uq *UserQuery) QueryReacts() *ReactQuery {
	query := &ReactQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryAuthentication() *AuthenticationQuery {
	query := &AuthenticationQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withReacts:         uq.withReacts.Clone(),
...
func (uq *UserQuery) WithReacts(opts ...func(*ReactQuery)) *UserQuery {
	query := &ReactQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithAuthentication(opts ...func(*AuthenticationQuery)) *UserQuery {
	query := &AuthenticationQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withReacts; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.React(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ReactsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_reacts
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_reacts" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_reacts" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withSubscriptions; query != nil {
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_subscriptions" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_subscriptions" returned %v for node %v`, *fk, n.ID)
			}
