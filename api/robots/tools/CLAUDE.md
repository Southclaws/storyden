...
### 12.3 Tool extension (`x-storyden-tool`)

Tools specify metadata via the `x-storyden-tool` extension object at the top of the YAML file:
...
- No role — service layer owns auth (e.g. `search`, `library_page_update`, `category_list`)

### 12.4 Code generation

After modifying tool schemas, regenerate the flat JSON:

```bash
task generate:mcp
```

Use `fnm env` if you need for `pnpm`.

This dereferences all `$ref` paths and inlines them (including `x-storyden-tool`) into `lib/mcp/robots.json`. The Go bindings in `lib/mcp/bindings.go` parse `x-storyden-tool` at runtime — no additional codegen step required.
...
- `api/robots/tools/*.yaml` - Individual tool definitions
- `lib/mcp/robots.json` - Dereferenced schema (generated, do not edit)
- `lib/mcp/bindings.go` - Go bindings with `x-storyden-tool` parsing
